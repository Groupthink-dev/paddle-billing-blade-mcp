...
Bearer token auth for remote/tunnel access. Set ``PADDLE_MCP_API_TOKEN`` env var.
...
def get_bearer_token() -> str | None:
    ...
    token = os.environ.get("PADDLE_MCP_API_TOKEN", "").strip()
...
class BearerAuthMiddleware:
    ...
    When ``PADDLE_MCP_API_TOKEN`` is set, every request must carry a matching
